...
int ape_parseheaderbuf(struct ape_ctx_t* ape_ctx, unsigned char* buf, int size)
{
	unsigned char* header;
	int offset = 0;

	if ((NULL == buf) || (NULL == ape_ctx) || (size <= 0))
	{
	...
	if (ape_ctx->fileversion >= 3980)
	{
		...
		offset = ape_ctx->descriptorlength + 24;
	...
	else
	{
		...
		ape_ctx->wavtaillength = get_uint32(buf + 20);
		...
		{
			offset += ape_ctx->wavheaderlength;
		}
...
int ape_parse_seek_table(struct ape_ctx_t* ape_ctx, unsigned char* buf, int size, int offset)
{
	int i;
	...
	if (size < (ape_ctx->totalframes * 4 + offset))
	{
		return -1;
	}

	ape_ctx->frames = malloc(ape_ctx->totalframes * sizeof(ape_frame_t));
	if (!ape_ctx->frames)
	{
		printf("<%s:%d> malloc frames error!\n", __func__, __LINE__);
		return -1;
	}

	offset += 4;	// seektable[0]
	ape_ctx->frames[0].pos     = ape_ctx->firstframe;
	ape_ctx->frames[0].nblocks = ape_ctx->blocksperframe;
	ape_ctx->frames[0].skip    = 0;
	for (i = 1; i < ape_ctx->totalframes; i++)
	{
		uint32_t seektable_entry = get_uint32(buf + offset);
		offset += 4;
		//printf("seektable_entry:%d\n", seektable_entry);
		ape_ctx->frames[i].pos      = seektable_entry + ape_ctx->junklength;
		ape_ctx->frames[i].nblocks  = ape_ctx->blocksperframe;
		ape_ctx->frames[i - 1].size = ape_ctx->frames[i].pos - ape_ctx->frames[i - 1].pos;
		ape_ctx->frames[i].skip     = (ape_ctx->frames[i].pos - ape_ctx->frames[0].pos) & 3;
	}
	offset += (ape_ctx->seektablelength / sizeof(uint32_t) - ape_ctx->totalframes);

	ape_ctx->frames[ape_ctx->totalframes - 1].nblocks = ape_ctx->finalframeblocks;
	//ape_ctx->frames[ape_ctx->total_frames - 1].size = ape_ctx->final_frame_blocks * 8;

	for (i = 0; i < ape_ctx->totalframes; i++)
	{
		if (ape_ctx->frames[i].skip)
		{
			ape_ctx->frames[i].pos  -= ape_ctx->frames[i].skip;
			ape_ctx->frames[i].size += ape_ctx->frames[i].skip;
		}
		ape_ctx->frames[i].size = (ape_ctx->frames[i].size + 3) & ~3;
	}
	
	if (ape_ctx->fileversion < 3810)
	{
		for (i = 0; i < ape_ctx->totalframes; i++)
		{
			int bits = buf[offset++];
			if (i && bits)
			{
				ape_ctx->frames[i - 1].size += 4;
			}

			ape_ctx->frames[i].skip <<= 3;
			ape_ctx->frames[i].skip  += bits;
		}
	}

	// calc pts, unit ms
	int64_t pts = 0;
	int duration = (ape_ctx->blocksperframe * 1000) / ape_ctx->samplerate;
	for (i = 0; i < ape_ctx->totalframes; i++)
	{
		ape_ctx->frames[i].pts = pts;
		pts += duration;
	}
...
void ape_dumpinfo(struct ape_ctx_t* ape_ctx)
{
	...
	printf("\nFrames totalframes:%d\n\n", ape_ctx->totalframes);
	//for (i = 0; i < ape_ctx->totalframes; i++)
	//	printf("%8d   %ld %d (%d samples)\n", i, ape_ctx->frames[i].pos, ape_ctx->frames[i].size, ape_ctx->frames[i].nblocks);
