...
int NeAACDecoderInit(NeAACDecHandle decoder)
{
	uint8_t i;
	NeAACDecStruct *hDecoder = (NeAACDecStruct *)decoder;
...
#endif
	hDecoder->frameLength = AAC_FRM_LEN;	//1024;
	...
	hDecoder->drc.ctrl1 = REAL_CONST(1.0);
	hDecoder->drc.ctrl2 = REAL_CONST(1.0);

	hDecoder->drc.num_bands = 1;
	hDecoder->drc.band_top[0] = 1024/4 - 1;
	hDecoder->drc.dyn_rng_sgn[0] = 1;
	hDecoder->drc.dyn_rng_ctl[0] = 0;
...
NeAACDecHandle NeAACDecOpen(void)
{
	...
	drc_init(&hDecoder->drc, REAL_CONST(1.0), REAL_CONST(1.0));
                              ...
                              unsigned long sample_buffer_size)
{
	...
	{
#endif
		raw_data_block(hDecoder, hInfo, &ld, &hDecoder->pce, &hDecoder->drc);	// this
#ifdef DRM
	}
#endif
...
#endif
		//printf("L%d size:%d/%d\n", __LINE__, sample_buffer_size, frame_len * output_channels * stride);
	...
	//printf("frm_id:%d frm_len:%d unsed:%d sample_num:%d\n", hDecoder->frame, hDecoder->frameLength, hInfo->bytesconsumed, hInfo->samples);
