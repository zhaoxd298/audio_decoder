...
int sbrDecodeInit(sbr_info *sbr, uint16_t framelength, uint8_t id_aac,
                        ...
                        )
{
    ...
    if (id_aac == ID_CPE)
    {
        /* stereo */
        uint8_t j;
        qmfa_init(&sbr->qmfa[0], 32);
        qmfa_init(&sbr->qmfa[1], 32);
        qmfs_init(&sbr->qmfs[0], (downSampledSBR)?32:64);
        qmfs_init(&sbr->qmfs[1], (downSampledSBR)?32:64);
#if 0
        for (j = 0; j < 5; j++)
        {
