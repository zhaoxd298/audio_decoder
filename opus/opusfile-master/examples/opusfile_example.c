...
void print_media_info(OggOpusFile *of, int n)
{
	const OpusHead *head;
	const OpusTags *tags;
...
void print_decode_info(OggOpusFile  *of, int n)
{
	...
	if (pcm_offset + n != next_pcm_offset)
	{
		fprintf(stderr, "\nPCM offset gap! %li+%i!=%li\n",
				(long)pcm_offset, n, (long)next_pcm_offset);
	}
...
int main(int _argc, const char **_argv)
{
	...
	if (_argc < 3)
	{
		fprintf(stderr, "Usage: %s <file.opus> <file.pcm>\n", _argv[0]);
		return EXIT_FAILURE;
	}
	...
	duration = 0;
	output_seekable = fseek(fout, 0, SEEK_CUR) != -1;
	...
	ret = fwrite(wav_header, sizeof(wav_header), 1, fout);
	if (0 == ret)
	{
	...
	for (;;)
	{
		...
		else if (ret < 0)
		{
			...
			if (is_ssl)
			{
				fprintf(stderr, "Possible truncation attack?\n");
			}
			ret = EXIT_FAILURE;
			break;
		}
		...
		print_media_info(of, ret);
		print_decode_info(of, ret);
		...
		if (!fwrite(out, sizeof(*out) * 4 * ret, 1, fout))
		{
