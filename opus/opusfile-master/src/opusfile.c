            ...
            const unsigned char *_initial_data, size_t _initial_bytes)
{
	...
	if (OP_UNLIKELY(_initial_bytes > (size_t)LONG_MAX))
	{
		return OP_EFAULT;
	}
	ogg_sync_init(&oy);
                                   ...
                                   opus_int64 _boundary)
{
	while (_boundary <= 0 || _of->offset < _boundary)
	{
		...
		/*printf("<%s:%d> ogg_sync_pageseek(&_of->oy, _og); more:%d\n", __func__, __LINE__, more);*/
                                 ...
                                 int *_cserialnos, ogg_page *_og)
{
	...
	while (ogg_page_bos(_og))
	{
		...
		if (_of->ready_state < OP_STREAMSET)
		{
			...
			printf("<%s:%d> ogg_stream_reset_serialno\n", __func__, __LINE__);
			ogg_stream_reset_serialno(&_of->os, ogg_page_serialno(_og));

			printf("<%s:%d> ogg_stream_pagein\n", __func__, __LINE__);
			ogg_stream_pagein(&_of->os, _og);

			printf("<%s:%d> ogg_stream_packetout\n", __func__, __LINE__);
			if (OP_LIKELY(ogg_stream_packetout(&_of->os, &op) > 0))
			{
				printf("<%s:%d> opus_head_parse\n", __func__, __LINE__);
				ret = opus_head_parse(_head, op.packet, op.bytes);
                                        ...
                                        opus_int64 _searched, opus_int64 _end_searched, opus_int32 _bias)
{
	...
	for (sri = 0; sri < _nsr; sri++)
	{
		...
		gp1 = _sr[sri].gp;
		if (gp1 < 0 || gp1 > OP_INT64_MAX - OP_GP_SPACING_MIN)
		{
			continue;
		}
...
static int op_make_decode_ready(OggOpusFile *_of)
{
	...
	{
		printf("<%s:%d> opus_multistream_decoder_ctl(_of->od, OPUS_RESET_STATE);\n", __func__, __LINE__);
		opus_multistream_decoder_ctl(_of->od, OPUS_RESET_STATE);
	}
	else
	{
		int err;
		printf("<%s:%d> opus_multistream_decoder_destroy();\n", __func__, __LINE__);
		opus_multistream_decoder_destroy(_of->od);
		
		printf("<%s:%d> opus_multistream_decoder_create();\n", __func__, __LINE__);
...
static int op_open_seekable2(OggOpusFile *_of)
{
	...
	*&os_start = _of->os;
	...
	printf("<%s:%d> ogg_sync_init()\n", __func__, __LINE__);
	ogg_sync_init(&_of->oy);

	printf("<%s:%d> ogg_stream_init(&_of->os, -1);\n", __func__, __LINE__);
	ogg_stream_init(&_of->os, -1);
	ret = op_open_seekable2_impl(_of);
	/*Restore the old stream state.*/
	printf("<%s:%d> ogg_stream_clear(&_of->os);\n", __func__, __LINE__);
	ogg_stream_clear(&_of->os);
	
	printf("<%s:%d> ogg_sync_clear(&_of->oy);\n", __func__, __LINE__);
                    ...
                    const unsigned char *_initial_data, size_t _initial_bytes)
{
	...
	*&_of->callbacks = *_cb;
	...
	printf("<%s:%d> ogg_sync_init(&_of->oy);\n", __func__, __LINE__);
	...
	if (_initial_bytes > 0)
	{
		...
		printf("<%s:%d> ogg_sync_wrote:%d\n", __func__, __LINE__, _initial_bytes);
...
OggOpusFile *op_open_file(const char *_path, int *_error)
{
	...
	void *stream = op_fopen(&cb, _path, "rb");
	
	return op_open_close_on_failure(stream, &cb, _error);
}
                     ...
                     const ogg_packet *_op, int _nsamples, int _nchannels)
{
	...
	if (ret == OP_DEC_USE_DEFAULT)
	{
		/*use this*/
#if defined(OP_FIXED_POINT)
        /*printf("<%s:%d> opus_multistream_decode()\n", __func__, __LINE__);*/
